...
UPawnCombatComponent* UWarriorFunctionLibrary::NativeGetPawnCombatComponentFromActor(AActor* InActor)
{
	...
	if (IPawnCombatInterface* PawnComabatInterface = Cast<IPawnCombatInterface>(InActor))
	{
		return PawnComabatInterface->GetPawnCombatComponent();
	}
	return nullptr;
}
...
bool UWarriorFunctionLibrary::IsTargetPawnHostile(APawn* QueryPawn, APawn* TargetPawn)
{
	...
	IGenericTeamAgentInterface* TargeteamAgent = Cast<IGenericTeamAgentInterface>(TargetPawn->GetController());

	if (QueryTeamAgent && TargeteamAgent)
	{
		return (QueryTeamAgent->GetGenericTeamId() != TargeteamAgent->GetGenericTeamId());
	}

	return false;
}

float UWarriorFunctionLibrary::GetScalebleFloatValueAtLevel(const FScalableFloat& InScalableFloat, float InLevel)
{
